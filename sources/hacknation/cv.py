...
# Orchestrators sleep this long between fetches - gentle on the CV host.
CV_FETCH_DELAY_S: Final[float] = 0.5
CV_FETCH_TIMEOUT_S: Final[float] = 30.0
...
DEFAULT_LLM_ENDPOINT: Final[str] = "databricks-claude-sonnet-4-5"
...
def fetch_cv(url: str, *, client: httpx.Client) -> bytes | None:
    """Fetch one CV PDF; a failed fetch must never crash the run.

    Args:
        url: The participant's cv_url.
        client: Shared HTTP client (callers sleep CV_FETCH_DELAY_S between calls).
    ...
    try:
        response = client.get(
            ...
            timeout=CV_FETCH_TIMEOUT_S,
            follow_redirects=True,
        )
    except (httpx.HTTPError, httpx.InvalidURL):
        return None
    content_type = response.headers.get("content-type", "").split(";")[0].strip().lower()
    if response.status_code != _HTTP_OK or content_type not in _PDF_CONTENT_TYPES:
        return None
